...
int __fillbuf(CC_FILE *fp)
{
	...
	if ((fp->flag & (__READ | __EOF | __ERR)) != __READ) /* flag is not __READ */
		return __EOF;

	bufsize = (fp->flag & __UNBUF) ? 1 : BUFFSIZE;
	if (fp->base == NULL) /* no buffer yet */
		if ((fp->base = cc_malloc(bufsize)) == NULL) /* can't get buffer */
			return EOF;
	fp->ptr = fp->base;
	fp->cnt = read(fp->fd, fp->ptr, bufsize);
	if (--fp->cnt < 0) {
		if (fp->cnt == -1)
			fp->flag |= __EOF; /* set flag to __EOF */
		else
			fp->flag |= __ERR; /* set flag to __ERR */
		fp->cnt = 0;
		return EOF;
	}
...
int __flushbuf(int x, CC_FILE *fp)
{
	...
	if ((fp->flag & (__WRITE | __ERR)) != __WRITE) /* flag is not __WRITE */
		return __ERR;
	...
	if (fp->base == NULL) { /* no buffer yet */
		...
		if (write(fp->fd, fp->base, bufsize) != bufsize) { /* write error */
			fp->flag |= __ERR; /* set flag to __ERR */
			return EOF;
		}
	}
...
/* cc_fflush: allocate and flush buffer of file poiter */
int cc_fflush(CC_FILE *fp)
{
	...
	bufsize = (fp->flag & __UNBUF) ? 1 : BUFFSIZE;
	wb = bufsize - fp->cnt; /* number of byte in buffer is allowed to write */
	if (write(fp->fd, fp->base, wb) != wb) { /* write erro */
		fp->flag |= __ERR; /* set __ERR bit on */
		return EOF;
	}
	fp->ptr = fp->base;
	fp->cnt = bufsize;
...
{
	if (fp->flag & __WRITE) /* flush buffer if flag is __WRITE */
		cc_fflush(fp);

	if (fp->base != NULL) /* free buffer */
		cc_free(fp->base);
...
/* cc_getfloat: floating-point of getint */
int cc_getfloat(float *fp)
{
	...
	while ((c = cc_getch()) == ' ' || c == '\t');
	if (!cc_isdigit(c) && c != '.' && c != '-' && c != '+') { /* not a number */
		cc_ungetch(c);
		return 0;
	}
	...
	if (c == '-' || c == '+')
		c = cc_getch();
	*fp = 0.0;
	while (cc_isdigit(c)) { /* collect integer part */
		*fp = *fp * 10.0 + c - '0';
		c = cc_getch();
	}
	power = 1.0;
	if (c == '.' && cc_isdigit(c = cc_getch())) /* collect fraction part */
		while(cc_isdigit(c)) {
			*fp = *fp * 10 + c - '0';
			power *= 10.0;
			c = cc_getch();
...
/* cc_getch: get a character from standard buffer */
int cc_getch(void)
{
	return (__bufp > 0) ? __buf[--__bufp] : cc_getchar();
}

/* cc_ungetch: push character back to standard buffer */
...
/* cc_fprint: print to file stream fp */
